...
const LoginBoxed = () => {
  ...
  const submitForm = async (event: React.FormEvent) => {
    ...
    try {
      ...
      await login(Login).then((response) => {
        console.log(response.status);
        if (response.status == 200) {
          const { name, email, token } = response.data.content;
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-cover bg-center bg-[url('/assets/images/map.svg')] dark:bg-[url('/assets/images/map-dark.svg')]">
      <div className="panel sm:w-[480px] m-6 max-w-lg w-full">
        ...
        <div className="relative my-2 h-5 text-center before:w-full before:h-[1px] before:absolute before:inset-0 before:m-auto before:bg-[#ebedf2] dark:before:bg-[#253b5c]"></div>
